...
- (IBAction)selectCarInsOrderStateButtonEvent:(UIButton *)sender
{
    ...
    {
        m_OrderStatus = [@(ENUM_MyOrder_Car_Status_WaitPay) stringValue];
    }
    if ([sender isEqual:self.carHadMoneyBtn])
    {
        m_OrderStatus = [@(ENUM_MyOrder_Car_Status_HadMoney) stringValue];
    }
    if ([sender isEqual:self.carWaitReviewBtn])
    {
        m_OrderStatus = [@(ENUM_MyOrder_Car_Status_WaitReview) stringValue];
    }
    if ([sender isEqual:self.carFinishBtn])
    {
        m_OrderStatus = [@(ENUM_MyOrder_Car_Status_Finish) stringValue];
    }
    [m_BaseView loadTableView];
}
