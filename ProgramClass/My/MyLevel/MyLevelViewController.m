...
- (void)viewDidLoad
{
    ...
    [self setLeftItemWithContentName:@"返回箭头-白"];
...
- (void)loadMyLevelDataToViewWithData:(NSDictionary *)data
{
    ...
    self.businessView.height = 61 + 28*(num-1);

    attrsRect = [m_levelModel.brokerBenefit boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 9999)
                                                         options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
                                                      attributes:@{NSFontAttributeName : DIF_DIFONTOFSIZE(20)}
                                                         context:nil];
    num = ceil(attrsRect.size.width/self.brokerContentView.width);
    [self.brokerView setTop:self.businessView.bottom];
    self.brokerView.height = 61 + 28*(num-1);

    attrsRect = [m_levelModel.juniorBenefit boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 9999)
                                                         options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
                                                      attributes:@{NSFontAttributeName : DIF_DIFONTOFSIZE(20)}
                                                         context:nil];
    num = ceil(attrsRect.size.width/self.juniorContent.width);
    [self.juniorView setTop:self.brokerView.bottom];
    self.juniorView.height = 61 + 28*(num-1);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [strongSelf.businessContentView setHeight:28*num];
        ...
        attrsRect = [strongSelf->m_levelModel.brokerBenefit boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 9999)
                                                          ...
                                                          attributes:@{NSFontAttributeName : DIF_DIFONTOFSIZE(20)}
                                                             context:nil];
        num = ceil(attrsRect.size.width/self.brokerContentView.width);
        [strongSelf.brokerContentView setHeight:28*num];
        ...
        [strongSelf.juniorContent setHeight:28*num];
