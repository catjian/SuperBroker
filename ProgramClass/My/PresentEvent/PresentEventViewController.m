...
- (void)httpRequestWithDrawalRequest
{
    ...
    if (self.MoneyTF.text.floatValue < [DIF_APPDELEGATE.mybrokeramount[@"minWithdrawalAmount"] floatValue])
    {
        [self.view makeToast:@"输入提现金额小于最小提现金额" duration:2 position:CSToastPositionCenter];
        return;
    }
    if (self.MoneyTF.text.floatValue > [DIF_APPDELEGATE.mybrokeramount[@"income"] floatValue])
    {
        [self.view makeToast:@"提现金额不能大于可用余额" duration:2 position:CSToastPositionCenter];
        return;
    }
    [CommonHUD showHUD];
