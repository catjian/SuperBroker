...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        NSDictionary *keyValue = @{@"默认排序":@"55",@"时间排序":@"56"};
        if (self.orderTypeStr &&
            [[self.orderTypeStr componentsSeparatedByString:@","] indexOfObject:keyValue[detailModel.dictName]] != NSNotFound)
        {
    ...
    {
        NSDictionary *keyValue = @{@"不限":@"57",@"消费型":@"54",@"返还型":@"46"};
        if (self.consumptionTypeStr &&
            [[self.consumptionTypeStr componentsSeparatedByString:@","] indexOfObject:keyValue[detailModel.dictName]] != NSNotFound)
        {
            [cell.titleLab setTextColor:DIF_HEXCOLOR(@"017aff")];
            [cell.layer setBorderColor:DIF_HEXCOLOR(@"017aff").CGColor];
            [m_selectIndexPath addObject:indexPath];
