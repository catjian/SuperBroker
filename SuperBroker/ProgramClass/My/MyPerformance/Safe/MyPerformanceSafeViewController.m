//
//  MyPerformanceSafeViewController.m
//  SuperBroker
//
...
#import "MyPerformanceSafeViewController.h"
#import "MyPerformanceSafeBaseView.h"

@interface MyPerformanceSafeViewController ()

@end

@implementation MyPerformanceSafeViewController
{
    MyPerformanceSafeBaseView *m_BaseView;
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (!m_BaseView)
    {
        m_BaseView = [[MyPerformanceSafeBaseView alloc] initWithFrame:CGRectMake(0, 12, self.view.width, self.view.height-12) style:UITableViewStylePlain];
...
- (void)httpRequestGetInviteListWithPageNumber:(NSInteger)pageNum
{
    ...
    [DIF_CommonHttpAdapter
     httpRequestMyPerformanceSafeWithParameters:@{@"pageNum":[@(pageNum) stringValue],@"pageSize":@"10"}
     ResponseBlock:^(ENUM_COMMONHTTP_RESPONSE_TYPE type, id responseModel) {
         ...
         if(type == ENUM_COMMONHTTP_RESPONSE_TYPE_SUCCESS)
         {
             ...
             MyPerformanceSafeListModel *listModle = [MyPerformanceSafeListModel mj_objectWithKeyValues:responseModel[@"data"]];
             if (pageNum == 1)
             {
