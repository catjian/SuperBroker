//
//  MyPerformanceCreditViewController.m
//  SuperBroker
//
...
#import "MyPerformanceCreditViewController.h"
#import "MyPerformanceCreditBaseView.h"

@interface MyPerformanceCreditViewController ()

@end

@implementation MyPerformanceCreditViewController
{
    MyPerformanceCreditBaseView *m_BaseView;
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (!m_BaseView)
    {
        m_BaseView = [[MyPerformanceCreditBaseView alloc] initWithFrame:CGRectMake(0, 12, self.view.width, self.view.height-12) style:UITableViewStylePlain];
...
- (void)httpRequestGetInviteListWithPageNumber:(NSInteger)pageNum
{
    ...
    [DIF_CommonHttpAdapter
     httpRequestMyPerformanceCreditWithParameters:@{@"pageNum":[@(pageNum) stringValue],@"pageSize":@"10"}
     ResponseBlock:^(ENUM_COMMONHTTP_RESPONSE_TYPE type, id responseModel) {
         ...
         if(type == ENUM_COMMONHTTP_RESPONSE_TYPE_SUCCESS)
         {
             ...
             MyPerformanceCreditListModel *listModle = [MyPerformanceCreditListModel mj_objectWithKeyValues:responseModel[@"data"]];
             if (pageNum == 1)
             {
